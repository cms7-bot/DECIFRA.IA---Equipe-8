...
| `RNF04` | **"Engine"** | A solução não deve utilizar uma engine de jogos. |
| `RNF05` | **"Modularidade"** | Os componentes devem possuir responsabilidades separadas, evitando concentrar interface, estado e regras em um único módulo. |
| `RNF06` | **"Consistência"** | As mesmas decisões, considerando o mesmo estado, devem produzir resultados consistentes de acordo com as regras definidas. |
| `RNF07` | **"Testabilidade"** | As principais regras e comportamentos do sistema devem poder ser testados de forma independente. |
| `RNF08` | **"Conteúdo educativo"** | A Unidade 1 deve abordar os quatro pilares definidos para o jogo: viés, alucinação, privacidade e dependência. |
| `RNF09` | **"Escopo técnico"** |: A solução não deve utilizar banco de dados, multiplayer, IA generativa/real ou engine de jogos. |
